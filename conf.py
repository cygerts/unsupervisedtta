...
def load_cfg_from_args(description="Config options."):
    ...
    #print("dataset ", cfg.CORRUPTION.DATASET)
    if cfg.CORRUPTION.DATASET in ["cifar10_c", "cifar10_1"]:
    ...
    if cfg.SETTING == "correlated":
        dataset_name = cfg.CORRUPTION.DATASET + "correl_" + str(cfg.TEST.ALPHA_DIRICHLET)
    else:
        dataset_name = cfg.CORRUPTION.DATASET
        num_repeats = int(cfg.REPEAT) 
        if num_repeats > 1:
            dataset_name = dataset_name + "_x{}".format(num_repeats)
        if not cfg.TEST.BATCH_SIZE in [1, 10]: #1 for MEMO, 10 as a standard
            dataset_name = dataset_name + "_bs{}".format(cfg.TEST.BATCH_SIZE)    

    #cfg.SAVE_DIR = os.path.join(cfg.SAVE_DIR, f"{cfg.MODEL.ADAPTATION}_{cfg.CORRUPTION.DATASET}_{current_time}")
    if cfg.MODEL.ADAPTATION in ["tent", "lame", "adacontrast", "memo"]:
        cfg.SAVE_DIR = os.path.join(cfg.SAVE_DIR, f"{dataset_name}/seed_{cfg.RNG_SEED}/{cfg.MODEL.ADAPTATION}/step_{cfg.OPTIM.STEPS}/LR_{cfg.OPTIM.LR:.8f}/MOMENTUM_{cfg.OPTIM.MOMENTUM}")
    elif cfg.MODEL.ADAPTATION == "cotta":
        cfg.SAVE_DIR = os.path.join(cfg.SAVE_DIR, f"{dataset_name}/seed_{cfg.RNG_SEED}/{cfg.MODEL.ADAPTATION}/step_{cfg.OPTIM.STEPS}/LR_{cfg.OPTIM.LR:.8f}/MOMENTUM_{cfg.OPTIM.MOMENTUM}/RST_{cfg.COTTA.RST}/AP_{cfg.COTTA.AP}")
    elif cfg.MODEL.ADAPTATION == "sar":
        cfg.SAVE_DIR = os.path.join(cfg.SAVE_DIR, f"{dataset_name}/seed_{cfg.RNG_SEED}/{cfg.MODEL.ADAPTATION}/step_{cfg.OPTIM.STEPS}/LR_{cfg.OPTIM.LR:.8f}/MOMENTUM_{cfg.OPTIM.MOMENTUM}/RST_{cfg.SAR.RESET_CONSTANT_EM}/E0_{cfg.SAR.E0_MULT}")
    elif cfg.MODEL.ADAPTATION == "eata":
        cfg.SAVE_DIR = os.path.join(cfg.SAVE_DIR, f"{dataset_name}/seed_{cfg.RNG_SEED}/{cfg.MODEL.ADAPTATION}/step_{cfg.OPTIM.STEPS}/LR_{cfg.OPTIM.LR:.8f}/MOMENTUM_{cfg.OPTIM.MOMENTUM}/DMAR_{cfg.EATA.D_MARGIN}/E0_{cfg.EATA.E0_MULT}/FISHER_{cfg.EATA.FISHER_ALPHA}")
    elif cfg.MODEL.ADAPTATION == "rmt":
        if float(cfg.RMT.LAMBDA_CE_SRC) > 0.:
            cfg.SAVE_DIR = os.path.join(cfg.SAVE_DIR, f"{dataset_name}/seed_{cfg.RNG_SEED}/{cfg.MODEL.ADAPTATION}/step_{cfg.OPTIM.STEPS}/LR_{cfg.OPTIM.LR:.8f}/MOMENTUM_{cfg.OPTIM.MOMENTUM}/SRC_{cfg.SOURCE.PERCENTAGE}")
        else:
            cfg.SAVE_DIR = os.path.join(cfg.SAVE_DIR, f"{dataset_name}/seed_{cfg.RNG_SEED}/{cfg.MODEL.ADAPTATION}_SF/step_{cfg.OPTIM.STEPS}/LR_{cfg.OPTIM.LR:.8f}/MOMENTUM_{cfg.OPTIM.MOMENTUM}/SRC_{cfg.SOURCE.PERCENTAGE}") #SOURCE-FREE version
    elif cfg.MODEL.ADAPTATION == "source":
        cfg.SAVE_DIR = os.path.join(cfg.SAVE_DIR, f"{dataset_name}/seed_{cfg.RNG_SEED}/{cfg.MODEL.ADAPTATION}/")
    else:
    ...
    assert(cfg.TEST.BATCH_SIZE in [1, 10, 32, 64, 128])
    print("BATCH SIZE ", cfg.TEST.BATCH_SIZE)
    assert(cfg.OPTIM.METHOD == 'SGD')
    print(cfg.MODEL.ARCH)
    if cfg.CORRUPTION.DATASET in ['cifar10_c', 'cifar10_1']:
        assert(cfg.MODEL.ARCH == 'Standard')
    elif cfg.CORRUPTION.DATASET == 'cifar100_c':
        assert(cfg.MODEL.ARCH == 'Hendrycks2020AugMix_ResNeXt')
    else:
...
def get_num_classes(dataset_name):
    dataset_name2num_classes = {"cifar10": 10, "cifar10_c": 10, "cifar10_1": 10, "cifar100": 100,  "cifar100_c": 100,
