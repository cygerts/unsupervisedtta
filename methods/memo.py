...
class MEMO(TTAMethod):
    def __init__(self, cfg, model, num_classes):
        ...
        print("Memo - Self episodic,", self.episodic)

    def forward(self, x, disable_adaptation=False):
        if self.episodic:
            self.reset()

        for _ in range(self.steps):
            if not disable_adaptation:
                x_aug = tta(x, self.n_augmentations, aug=self.augmentations, device="cuda")
    ...
    def configure_model(self):
        self.model = AlphaBatchNorm.adapt_model(self.model, alpha=self.alpha_bn).to("cuda")
