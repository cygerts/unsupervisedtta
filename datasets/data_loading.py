...
def get_transform(dataset_name, adaptation):
    ...
    if adaptation == "adacontrast":
        ...
        if dataset_name in {"cifar10", "cifar100", "cifar10_c", "cifar100_c", "cifar10.1"}:
        ...
        if dataset_name in {"cifar10", "cifar10.1", "cifar100"}:
...
def get_test_loader(setting, adaptation, dataset_name, root_dir, domain_name, severity, num_examples,
    ...
    if domain_name == "none":
        ...
        if dataset_name in {"cifar10_c", "cifar100_c"}:
            ...
            label_filepath = os.path.join(data_dir, "cifar10.1_v4_labels.npy")
            imagedata_filepath = os.path.join(data_dir, "cifar10.1_v4_data.npy")
            labels = np.load(label_filepath)
            data = np.load(imagedata_filepath)

            data_transform = transforms.ToTensor()
            transformed_features = torch.stack([data_transform(feature) for feature in data])
            test_dataset = torch.utils.data.TensorDataset(transformed_features, torch.tensor(labels))
    ...
    return torch.utils.data.DataLoader(test_dataset, batch_size=batch_size, shuffle=shuffle, num_workers=workers, drop_last=True)
...
def get_source_loader(dataset_name, root_dir, adaptation, batch_size, train_split=True, ckpt_path=None, num_samples=None, percentage=1.0, workers=4):
    ...
    dataset_name = dataset_name.split("_")[0] if dataset_name in {"cifar10_c", "cifar10_1", "cifar100_c", "imagenet_c", "imagenet_k"} else dataset_name
    ...
    elif dataset_name in {"imagenet_r", "imagenet_a", "imagenet_d", "imagenet_d109", "imagenet_v2"}:
        split = "train" if train_split else "val"
